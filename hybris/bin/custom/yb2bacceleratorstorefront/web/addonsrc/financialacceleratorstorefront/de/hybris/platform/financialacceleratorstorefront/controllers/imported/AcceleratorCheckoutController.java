...
public class AcceleratorCheckoutController extends AbstractCheckoutController
{
	...
	private static final Logger LOGGER = Logger.getLogger(AcceleratorCheckoutController.class);
			...
			throws CMSItemNotFoundException
	{
		...
		CollectionUtils.emptyIfNull(orderDetails.getEntries()).forEach(entry ->
		{
		...
		final CategoryData categoryData = orderDetails.getEntries().get(0).getProduct().getDefaultCategory();
		...
		final List<InsuranceQuoteReviewData> insuranceQuoteReviewDataList = insuranceQuoteReviewFacade.getInsuranceQuoteReviewsFromOrder(orderDetails);
		model.addAttribute("insuranceQuoteReviews",insuranceQuoteReviewDataList);
		model.addAttribute("isValidStep", true);
		...
		if (OrderStatus.CANCELLED.equals(orderDetails.getStatus()) || OrderStatus.CHECKED_INVALID.equals(orderDetails.getStatus()))
		{
			GlobalMessages.addErrorMessage(model, "checkout.error.invalid.order");
		}

		final String view = getViewForPage(model);
		return StringUtils.isNotEmpty(view) ? view : ControllerConstants.Views.Pages.Checkout.CheckoutConfirmationPage;
	}
