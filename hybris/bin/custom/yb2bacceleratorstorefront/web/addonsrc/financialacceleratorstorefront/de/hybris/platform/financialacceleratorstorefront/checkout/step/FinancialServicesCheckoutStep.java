...
public class FinancialServicesCheckoutStep extends AbstractInsuranceStepTransition
{
	...
	/**
	 * @param isCategoryUrl the isCategoryUrl to set
	 */
	 ...
	 * checkout/cart.
	 *
	 * @param strategy StepVisitedAnalysisStrategy that belongs to the current requested progress bar step
	...
	public boolean isSameCategory(final StepVisitedAnalysisStrategy strategy)
	{
		...
		if ((hasCategoryDataAndCategoryUrl(categoryData, categoryUrl) && !isCategoryUrl)
	...
	private boolean hasCategoryDataAndCategoryUrl(final CategoryData categoryData, final String categoryUrl)
	{
		return categoryData == null || StringUtils.isBlank(categoryUrl) || (StringUtils.isNotBlank(categoryUrl)
				&& categoryUrl.indexOf(categoryData.getCode()) > 0);
	}
