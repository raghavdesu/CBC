...
package de.hybris.platform.financialacceleratorstorefront.checkout.step.strategy.impl;
...
import de.hybris.platform.financialacceleratorstorefront.checkout.step.FormCheckoutStep;
import de.hybris.platform.financialacceleratorstorefront.checkout.step.strategy.CheckoutFormsStrategy;
import de.hybris.platform.financialacceleratorstorefront.checkout.step.strategy.DynamicFormCheckoutStrategy;
import de.hybris.platform.financialacceleratorstorefront.checkout.strategy.impl.InsuranceYFormDataPreprocessorStrategy;
...
import de.hybris.platform.financialacceleratorstorefront.yforms.EmbeddedFormXml;
import de.hybris.platform.financialacceleratorstorefront.yforms.EmbeddedFormXmlParser;
...
public class DefaultDynamicFormCheckoutStrategy implements DynamicFormCheckoutStrategy
{
	 ...
	 */
	@Override
	public List<FinancialServicesCheckoutStep> createDynamicFormSteps(final CheckoutStep formCheckoutStepPlaceholder)
	{
		final CartData cartData = getCheckoutFacade().getCheckoutCart();
		final List<FinancialServicesCheckoutStep> newSteps = new ArrayList<>();
		CheckoutStep previous = null;
		final Map<String, String> formPageIds = getCheckoutFormsStrategy().getFormPageIdList(cartData);
		for (final Map.Entry<String, String> formPageIdEntry : formPageIds.entrySet())
		{
			final FormCheckoutStep newStep = createNewCheckoutStep(formCheckoutStepPlaceholder, previous,
					formPageIdEntry.getKey(), formPageIdEntry.getValue());
			previous = newStep;
			...
			final String formPageId, final String progressBarId)
	{
		...
		if (formCheckoutStepPlaceholder instanceof FinancialServicesCheckoutStep)
		{
			newStep.setStepVisitedAnalysisStrategyList(
					((FinancialServicesCheckoutStep) formCheckoutStepPlaceholder).getStepVisitedAnalysisStrategyList());
			newStep.setCheckoutStepValidatorList(
					((FinancialServicesCheckoutStep) formCheckoutStepPlaceholder).getCheckoutStepValidatorList());
			newStep.setStepEnabledAnalysisStrategyList(
					((FinancialServicesCheckoutStep) formCheckoutStepPlaceholder).getStepEnabledAnalysisStrategyList());
		}
			...
			final List<AbstractCheckoutStepController.CheckoutSteps> originalCheckoutSteps)
	{
		...
		final List<FinancialServicesCheckoutStep> formCheckoutSteps = createDynamicFormSteps(formCheckoutStepPlaceholder);
		int progressKey = 0;
		for (final AbstractCheckoutStepController.CheckoutSteps checkoutStep : originalCheckoutSteps)
		{
			...
			{
				for (final FinancialServicesCheckoutStep formCheckoutStep : formCheckoutSteps)
				{
			...
			else
			{
				...
				{
					final AbstractInsuranceCheckoutStepController.InsuranceCheckoutSteps step =
							(AbstractInsuranceCheckoutStepController.InsuranceCheckoutSteps) checkoutStep;
					combinedCheckoutSteps.add(
							new AbstractInsuranceCheckoutStepController.InsuranceCheckoutSteps(checkoutStep.getProgressBarId(),
									step.getUrl(), progressKey, step.getStatus(), step.getIsEnabled()));
				}
				else
				{
					combinedCheckoutSteps.add(
							new AbstractInsuranceCheckoutStepController.InsuranceCheckoutSteps(checkoutStep.getProgressBarId(),
									checkoutStep.getUrl(), progressKey));
				}
				progressKey++;
			}
		}
	...
	public List<String> getFormsInlineHtmlByFormPageId(final String formPageId)
	{
		...
		for (final FormDetailData data : forms)
		{
			...
			try
			{
				...
				embeddedFormHtml = getYFormFacade().getInlineFormHtml(data.getApplicationId(), data.getFormId(), action,
						formDataId, strategy, params);
	...
	{
		List<EmbeddedFormXml> datas = sessionService.getAttribute(EmbeddedFormXml.EMBEDDED_FORM_XMLS);
