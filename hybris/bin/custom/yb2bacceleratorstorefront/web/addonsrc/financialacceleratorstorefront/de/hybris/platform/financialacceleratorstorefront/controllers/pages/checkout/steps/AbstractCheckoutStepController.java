...
public abstract class AbstractCheckoutStepController extends AbstractCheckoutController implements CheckoutStepController
{
	...
	protected static final String LEGAL_INFORMATION_PAGE_LABEL = "legalInformationPage";
	protected static final String USER_IDENTIFICATION_PAGE_LABEL = "userIdentificationPage";
	protected static final String REDIRECT_ADD_PAYMENT_METHOD_KEY = "payment.method.add.url";
	...
	public List<CheckoutSteps> addCheckoutStepsToModel()
	{
		...
		for (final Map.Entry<String, CheckoutStep> entry : progressBarMap.entrySet())
		{
			...
			checkoutSteps.add(new CheckoutSteps(checkoutStep.getProgressBarId(),
					StringUtils.remove(checkoutStep.currentStep(), REDIRECT_STRING), Integer.valueOf(entry.getKey())));
		}
	...
	protected void setCheckoutStepLinksForModel(final Model model, final CheckoutStep checkoutStep)
	{
		...
		{
			model.addAttribute("previousStepUrl", StringUtils.remove(checkoutStep.previousStep(), REDIRECT_STRING));
			model.addAttribute("nextStepUrl", StringUtils.remove(checkoutStep.nextStep(), REDIRECT_STRING));
			model.addAttribute("currentStepUrl", StringUtils.remove(checkoutStep.currentStep(), REDIRECT_STRING));
	...
	protected boolean isPaymentEnabledForCurrentSite(CartData cartData)
	{
		final String storeName = cartData.getSite();
		final String propertyPaymentEnabled = WebConstants.FINANCIAL_STOREFRONT + "." + storeName + "." + WebConstants
				.PAYMENT_ENABLED;

		return getConfigurationService().getConfiguration().getBoolean(propertyPaymentEnabled, false);
	}
	...
	protected String getRedirectAddPaymentInfoUrl()
	{
		return getConfigurationService().getConfiguration()
				.getString(REDIRECT_ADD_PAYMENT_METHOD_KEY, StringUtils.EMPTY);
	}
