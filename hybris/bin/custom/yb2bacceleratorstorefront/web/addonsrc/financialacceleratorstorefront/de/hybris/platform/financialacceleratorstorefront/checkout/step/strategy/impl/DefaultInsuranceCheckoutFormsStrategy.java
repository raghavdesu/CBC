...
import de.hybris.platform.financialacceleratorstorefront.checkout.step.strategy.AbstractCheckoutFormsStrategy;
...
public class DefaultInsuranceCheckoutFormsStrategy extends AbstractCheckoutFormsStrategy
{
	...
	{
		final Map<String, String> formPageIds = new HashMap<>();

		if (cartData != null && CollectionUtils.isNotEmpty(cartData.getEntries()))
		{
			final int steps = Math.min(cartData.getEntries().size(), getMaxCheckoutFormPages());
