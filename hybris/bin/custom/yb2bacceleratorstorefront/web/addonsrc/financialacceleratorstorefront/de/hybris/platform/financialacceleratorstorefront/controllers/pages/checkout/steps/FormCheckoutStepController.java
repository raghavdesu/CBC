...
public class FormCheckoutStepController extends AbstractInsuranceCheckoutStepController
{
	...
	private static final Logger LOG = Logger.getLogger(FormCheckoutStepController.class);
			...
			final String messageHolderType)
	{
		if (model != null && redirectAttributes != null && model.containsAttribute(messageHolderType))
		{
			...
			if (obj instanceof List && CollectionUtils.isNotEmpty((List) obj))
			{
				...
				{
					final GlobalMessage globalMessage = (GlobalMessage) o;
					final Collection attributes = globalMessage.getAttributes();
					GlobalMessages.addFlashMessage(redirectAttributes, messageHolderType, globalMessage.getCode(),
			...
			final RedirectAttributes redirectAttributes, final HttpServletRequest request) throws CMSItemNotFoundException
	{
		...
		final List<FinancialServicesCheckoutStep> formCheckoutSteps = getDynamicFormCheckoutStrategy()
	...
	{
		final List<FinancialServicesCheckoutStep> dynamicFormProgressSteps = getDynamicFormCheckoutStrategy()
				.createDynamicFormSteps(getFormPlaceholder());
		...
		if (refererURL.contains(StringUtils.remove(getCheckoutStep().nextStep(), "redirect:")))
		{
			...
			if (!dynamicFormProgressSteps.isEmpty())
			{
	...
	protected void setVisitedToStep(final CheckoutStep checkoutStep)
	{
		...
		if (getFormPlaceholder() instanceof FinancialServicesCheckoutStep)
		{
			((FinancialServicesCheckoutStep) getFormPlaceholder()).setVisited(true);
		}
	}
	...
	protected void setupFormsToModel(final Model model, final String formPageId)
	{
		 ...
		 */
		final List<EmbeddedFormXml> xmls = getSessionService()
