...
<div class="asm__customer360-header clearfix">
    <div class="asm__customer360-header-image">
        <c:choose>
            <c:when test="${not empty headingData.profilePicture}">
                ...
                <img src="${profilePictureUrl}" class="asm__customer360-customer-image img-circle" alt=""
    ...
    <div class="asm__customer360-header-desc">
        ...
        <div class="asm__customer360-header-email">
            <a href="mailto:${fn:escapeXml(headingData.email)}"> <c:out value="${fn:escapeXml(headingData.email)}"/> </a>
        </div>
                ...
                <span class="asm__header__cart">
                    <c:choose>
                        <c:when test="${headingData.cartSize gt 0}">
                            ...
                            <span class="hidden-xs hidden-sm hidden-md"><spring:theme code="text.customer360.headline.cart" arguments="${headingData.cartSize}"/>&nbsp;</span><a href="${link}"><c:out value='${fn:escapeXml(headingData.cartCode)}'/></a>
                        </c:when>
                        <c:otherwise>
                ...
                <span class="asm__header__order">
                    <c:choose>
                        <c:when test="${not empty headingData.latestOrderCode}">
							<c:set var="latestOrderCode" value="${headingData.latestOrderCode}"/>
                            <spring:url value="/my-account/order/${latestOrderCode}" var="link" htmlEscape="false"/>
                            <span class="hidden-xs hidden-sm hidden-md"><spring:theme code="text.customer360.headline.order"/>&nbsp;</span>${fn:escapeXml(headingData.latestOrderTotal)},&nbsp;<a>${fn:escapeXml(latestOrderCode)}</a>,
                ...
                <span class="asm__header__ticket">
                    <c:choose>
                        <c:when test="${not empty headingData.latestOpenedTicketId}">
                     	    <c:set var="latestOpenedTicketId" value="${headingData.latestOpenedTicketId}"/>
                            <spring:url value="/my-account/support-ticket/${latestOpenedTicketId}" var="link"
                                        htmlEscape="false"/>
                            <spring:theme code="text.customer360.headline.ticket"/> <a href="${link}">&nbsp;${fn:escapeXml(headingData.latestOpenedTicketId)}</a>,
