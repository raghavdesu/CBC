...
<div class="yCmsComponent col-md-12">
    <c:if test="${not empty categoryName}">
        <h2 class="checkout-progress checkout-progress__heading"><spring:theme code="checkout.progress.bar.insurance.label.${fn:toLowerCase(cmsSite.channel)}" text="Your Insurance Quote is in progress" arguments="${categoryName}"/></h2>
    </c:if>

    <ul data-role="navbar" id="checkoutProgress" class="checkout-progress__body steps-${fn:length(steps)}">
        <c:forEach items="${steps}" var="checkoutStep" varStatus="status">
            ...
            <c:choose>
                <c:when test="${checkoutStep.status eq 'unset' or cartData.insuranceQuote.state eq 'BIND'}">
            ...
            <c:choose>
                <c:when test="${isEnabled == true}">
                    <c:set var="hrefLink" value="href='${fn:escapeXml(stepUrl)}'"/>
                </c:when>
                <c:otherwise>
            ...
            <c:choose>
                <c:when test="${checkoutStep.progressBarId eq 'paymentMethod' || checkoutStep.progressBarId eq 'confirmOrder' || checkoutStep.progressBarId eq 'legal-information' || checkoutStep.progressBarId eq 'user-identification' || cartData.insuranceQuote.state eq 'BIND'}">
            ...
            <c:choose>
                ...
                <c:when test="${checkoutStep.stepNumber > currentStepActive or cartData.insuranceQuote.state eq 'BIND'}">
                    <li class="checkout-progress__step js-step disabled ${currentStepIndicator}">
                        <c:choose>
                ...
                <c:otherwise>
                    <li class="checkout-progress__step js-step visited ${currentStepIndicator}">
                        <a ${hrefLink}>
                            <spring:theme code="checkout.multi.${checkoutStep.progressBarId}"/>
                        </a>
                    </li>
